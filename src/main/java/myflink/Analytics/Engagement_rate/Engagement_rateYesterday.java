...
public class Engagement_rateYesterday {
    ...
    public static Table resultER30 = ComputeTable(Stream);

    public static void main(String[] args) throws Exception {
        DataStream<Row> resultStreamER30 = TableToStream(resultER30);
        MongoDBSink_ctr mongoDBSink_ctr=new MongoDBSink_ctr("engagement_rateYesterday");
    ...
    public static Table ComputeTable (DataStream<Columns> n1) {
        ...
        Table resultTable2 = Env.tableEnv.sqlQuery(
                ...
                " SELECT BRAND AS BRAND2, WALL_ID AS WALL_ID2 , WALLGROUP_ID AS WALLGROUP_ID2, CAMPAIGN_ID AS CAMPAIGN_ID2, " +
                        "EVENT_TYPE AS EVENT_TYPE2,ISOTIMESTAMP AS ISOTIMESTAMP2, " +
                        " COUNT(EVENT_TYPE = 'post-impression') AS IMPRESSIONS29 " +
                        " FROM TABLE (TUMBLE(TABLE InputTable, DESCRIPTOR(ISOTIMESTAMP), INTERVAL '20' MINUTES))"+
